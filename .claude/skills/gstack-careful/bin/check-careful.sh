...
# Python fallback if grep returned empty or truncated (e.g., escaped quotes in command)
if [ -z "$CMD" ] || printf '%s' "$CMD" | grep -q '\\$' 2>/dev/null; then
...
# Strip quoted strings to avoid false positives (e.g., echo 'rm -rf /')
CMD_SAFE=$(printf '%s' "$CMD" | sed -E "s/'[^']*'//g;s/\"[^\"]*\"//g")

# Normalize: lowercase for case-insensitive SQL matching (uses CMD_SAFE to avoid false positives on quoted SQL)
CMD_SAFE_LOWER=$(printf '%s' "$CMD_SAFE" | tr '[:upper:]' '[:lower:]')
...
if printf '%s' "$CMD" | grep -qE 'rm\s+(-[a-zA-Z]*r[a-zA-Z]*\s+|--recursive\s+)' 2>/dev/null; then
  SAFE_ONLY=true
  RM_ARGS=$(printf '%s' "$CMD" | sed -E 's/^.*rm[[:space:]]+//;s/--[a-zA-Z][-a-zA-Z]*[[:space:]]+//g;s/-[a-zA-Z]+[[:space:]]+//g')
...
if printf '%s' "$CMD_SAFE" | grep -qE 'rm\s+(-[a-zA-Z]*r|--recursive)' 2>/dev/null; then
  WARN="Destructive: recursive delete (rm -r). This permanently removes files."
  PATTERN="rm_recursive"
fi

# DROP TABLE / DROP DATABASE — check CMD_SAFE_LOWER for unquoted SQL; also check raw CMD
# when a known SQL client is the command (psql, mysql, sqlite3, etc.) since SQL args are quoted
SQL_CMD_LOWER=$(printf '%s' "$CMD" | tr '[:upper:]' '[:lower:]')
IS_SQL_CLIENT=false
printf '%s' "$SQL_CMD_LOWER" | grep -qE '^\s*(psql|mysql|sqlite3|mongosh|mongo|sqlcmd|bq)\b' 2>/dev/null && IS_SQL_CLIENT=true

if [ -z "$WARN" ]; then
  if printf '%s' "$CMD_SAFE_LOWER" | grep -qE 'drop\s+(table|database)' 2>/dev/null; then
    WARN="Destructive: SQL DROP detected. This permanently deletes database objects."
    PATTERN="drop_table"
  elif [ "$IS_SQL_CLIENT" = true ] && printf '%s' "$SQL_CMD_LOWER" | grep -qE 'drop\s+(table|database)' 2>/dev/null; then
    WARN="Destructive: SQL DROP detected. This permanently deletes database objects."
    PATTERN="drop_table"
  fi
fi
...
# TRUNCATE
if [ -z "$WARN" ]; then
  if printf '%s' "$CMD_SAFE_LOWER" | grep -qE '\btruncate\b' 2>/dev/null; then
    WARN="Destructive: SQL TRUNCATE detected. This deletes all rows from a table."
    PATTERN="truncate"
  elif [ "$IS_SQL_CLIENT" = true ] && printf '%s' "$SQL_CMD_LOWER" | grep -qE '\btruncate\b' 2>/dev/null; then
    WARN="Destructive: SQL TRUNCATE detected. This deletes all rows from a table."
    PATTERN="truncate"
  fi
fi
