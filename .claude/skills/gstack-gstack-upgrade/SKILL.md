...
### Step 3: Save old version (REQUIRED before upgrading)

**You MUST capture the old version BEFORE running any upgrade commands.** This value is needed for the What's New summary and the upgrade marker in Step 5.
...
Do not proceed to Step 4 until you have saved `OLD_VERSION`.
...
Uses `OLD_VERSION` from Step 3. If you skipped Step 3, go back and read `VERSION` from the backup (`.bak`) before it's deleted.
...
2. If `UPGRADE_AVAILABLE <old> <new>`: follow Steps 2-6 above (including Step 4.5 — local vendored sync).

3. If no output (primary is up to date): **you MUST still check for a stale local vendored copy** — the local copy may be behind even when the global is current.

First, detect the primary install:
```bash
if [ -d "$HOME/.claude/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"; INSTALL_DIR="$HOME/.claude/skills/gstack"
elif [ -d ".claude/skills/gstack/.git" ]; then
  INSTALL_TYPE="local-git"; INSTALL_DIR=".claude/skills/gstack"
elif [ -d ".claude/skills/gstack" ]; then
  INSTALL_TYPE="vendored"; INSTALL_DIR=".claude/skills/gstack"
elif [ -d "$HOME/.claude/skills/gstack" ]; then
  INSTALL_TYPE="vendored-global"; INSTALL_DIR="$HOME/.claude/skills/gstack"
fi
echo "Install type: $INSTALL_TYPE at $INSTALL_DIR"
```

Then check for a local vendored copy at a different path:
```bash
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/.claude/skills/gstack" ]; then
...
**If versions differ:** sync the local copy from the primary:
```bash
mv "$LOCAL_GSTACK" "$LOCAL_GSTACK.bak"
cp -Rf "$INSTALL_DIR" "$LOCAL_GSTACK"
rm -rf "$LOCAL_GSTACK/.git"
...
rm -rf "$LOCAL_GSTACK.bak"
```
Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `.claude/skills/gstack/` when you're ready."
