...
Build reliable browser automation skills through iterative experimentation. In native mode (default), you drive the `browse` CLI directly and self-improve. In harness mode, an inner agent browses the site (`evaluate.mjs`). Either way, you — the outer agent — read what happened and improve the instructions (`strategy.md`). Repeat until it passes consistently.
...
Check that `./autobrowse/tasks/<task>/task.md` exists (scaffold it from the template if not — see Step 2). `strategy.md` is auto-created empty on first run.

### Choose a mode

**Native mode (default, no API key)**: Claude Code drives `browse` commands directly and self-improves. Use this when running under Claude Code with subscription auth.

**Harness mode (requires ANTHROPIC_API_KEY)**: The evaluate.mjs inner agent runs a separate Claude instance. Use this for parallel headless runs or CI.

### Native mode — run browse commands directly

1. Read the task definition and any existing strategy:
```bash
cat ./autobrowse/tasks/<task-name>/task.md
cat ./autobrowse/tasks/<task-name>/strategy.md 2>/dev/null || echo "(no strategy yet)"
```

2. Execute the task by running `browse` CLI commands via Bash, following the steps in task.md and any heuristics in strategy.md:
```bash
browse open <url>
browse wait load
browse snapshot
# ... follow the steps, clicking refs from snapshots
```

3. After completion, evaluate the result. Did it succeed? Save a trace summary:
```bash
mkdir -p ./autobrowse/traces/<task-name>/run-NNN
# Write a summary of what happened, what worked, what failed
```

4. Continue to "Form one hypothesis" below.

### Harness mode — run the inner agent
