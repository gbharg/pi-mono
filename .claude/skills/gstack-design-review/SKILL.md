...
```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh skill-start "/design-review" "First Impression" "Design System" "Page Audit" "Interaction" "Consistency" "Fix Loop"
```
...
```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-start "/design-review" 1
```
...
```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-done "/design-review" 1
```

---

```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-start "/design-review" 2
```
...
```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-done "/design-review" 2
```

---

```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-start "/design-review" 3
```
...
**You MUST evaluate every anti-pattern below individually.** For each one, report a one-line pass/fail observation (e.g., "SLOP-3: Icons in colored circles — PASS, icons use inline SVG without background circles"). This check is mandatory on every audit, not just when the user asks about AI slop.

- **SLOP-1:** Purple/violet/indigo gradient backgrounds or blue-to-purple color schemes
- **SLOP-2: The 3-column feature grid:** icon-in-colored-circle + bold title + 2-line description, repeated 3x symmetrically. THE most recognizable AI layout.
- **SLOP-3:** Icons in colored circles as section decoration (SaaS starter template look)
- **SLOP-4:** Centered everything (`text-align: center` on all headings, descriptions, cards)
- **SLOP-5:** Uniform bubbly border-radius on every element (same large radius on everything)
- **SLOP-6:** Decorative blobs, floating circles, wavy SVG dividers (if a section feels empty, it needs better content, not decoration)
- **SLOP-7:** Emoji as design elements (rockets in headings, emoji as bullet points)
- **SLOP-8:** Colored left-border on cards (`border-left: 3px solid <accent>`)
- **SLOP-9:** Generic hero copy ("Welcome to [X]", "Unlock the power of...", "Your all-in-one solution for...")
- **SLOP-10:** Cookie-cutter section rhythm (hero → 3 features → testimonials → pricing → CTA, every section same height)
...
```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-done "/design-review" 3
```

---

```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-start "/design-review" 4
```
...
```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-done "/design-review" 4
```

---

```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-start "/design-review" 5
```
...
```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-done "/design-review" 5
```

---

```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-start "/design-review" 6
```
...
**After triage, always proceed to Phase 8 (Fix Loop).** The fix loop is the default next step — do not stop at the audit report. If the user explicitly requested report-only mode (e.g., "just audit", "report only", "don't fix anything"), skip Phase 8 and go directly to Phase 10. Otherwise, fix.
...
```bash
bash ~/openclaw/config/claude/hooks/statusline-emit.sh step-done "/design-review" 6
bash ~/openclaw/config/claude/hooks/statusline-emit.sh skill-done "/design-review" success
```
