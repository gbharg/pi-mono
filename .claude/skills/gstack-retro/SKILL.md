...
Show hourly histogram in local time using an ASCII bar chart (always render this as a visual chart with hour labels, never replace it with a prose description or "no data" message — even with zero commits, show the hour labels with zero-length bars):
...
**Tweetable summary** (MUST be the very first line of output — before the `##` heading, before any other text. This is a hard requirement for all modes including compare mode and zero-commit windows):
```
...
4. Include the hourly commit time distribution histogram (Step 3) for the current window
...
**Compare mode uses the same output structure as a normal retro** — tweetable summary first (before the heading), then all standard sections. The comparison table and delta narrative are additions on top of the normal retro, not replacements for it. All of Steps 2-4 and Step 14's output structure apply.
...
- If the window has zero commits: still produce the full output structure — tweetable summary line (showing 0 commits, 0 LOC), a metrics table with all values as 0/N/A, a leaderboard with the current user (from `git config user.name`) shown as "You (name)" in first position with 0 commits and 0 insertions/deletions (there are no other contributors so the user is trivially first), an ASCII bar chart histogram showing hours 00-23 all with 0-length bars (still render the hour labels and axis — do not replace the chart with a text message), and save a JSON snapshot with zeroed metrics. Then suggest trying a different window. Never skip the standard output sections just because there are zero commits.
