...
Auto-generated by agent wrapper"
  ...
  if [[ -z "$pr_url" ]]; then
    ...
    pr_url=$(gh pr list --head "$AGENT_BRANCH" --json url --jq '.[0].url' 2>/dev/null || echo "")
  fi
...
Changes:
- $(git rev-list --count "$BASE_REF..$AGENT_BRANCH") commit(s)
- $(git diff --shortstat "$BASE_REF..$AGENT_BRANCH")"
